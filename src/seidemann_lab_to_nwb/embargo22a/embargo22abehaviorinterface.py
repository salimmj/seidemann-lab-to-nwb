...
class Embargo22ABehaviorInterface(BaseDataInterface):
    ...
    def run_conversion(self, nwbfile: NWBFile, metadata: dict):
        ...
        number_to_outcome_map = {
            number: outcome for outcome, number in definitions["OUTCOME"].items()
        }

        type_condition_to_description = {0: "blank", 1: "target", 2: "visual_stimulus"}
        conditions = header["Conditions"]
        conditions_type = [
            type_condition_to_description[type_cond]
            for type_cond in conditions["TypeCond"]
        ]
        current_condition_to_description = {
            cond + 1: description for cond, description in enumerate(conditions_type)
        }
        ...
        single_value_columns = [
            c for c in all_columns if len(df_trial_data[c].unique()) == 1
        ]
        ...
        df_trial_data["Outcome"] = [
            number_to_outcome_map[outcome].lower()
            for outcome in df_trial_data["Outcome"]
        ]
        df_trial_data["condition type"] = [
            current_condition_to_description[CurrCond]
            for CurrCond in df_trial_data["CurrCond"]
        ]

        # Time in seconds
        time_columns = [
            column
            for column in df_trial_data.columns
            if "Time" in column and "Now" not in column
        ]
        for column in time_columns:
            df_trial_data[column] = df_trial_data[column] / 1000.0

        # Order them by average distance from trial start
        df_aligned = df_trial_data[time_columns].sub(
            df_trial_data.TimeTrialStart, axis=0
        )
        ...
        # Re-name
        df_trial_data.rename(columns={"TimeTrialStart": "start_time", "TimeTrialEnd": "stop_time"}, inplace=True)
