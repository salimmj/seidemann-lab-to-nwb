...
class Embargo22ABehaviorInterface(BaseDataInterface):
    ...
    def run_conversion(self, nwbfile: NWBFile, metadata: dict):
        ...
        number_to_defintion_map = {number: outcome for outcome, number in definitions["EVENT"].items()}

        file_path_events = session_path / "events.csv"
        df_events = pd.read_csv(file_path_events)
        df_events["event_type"] = [number_to_defintion_map[number].lower() for number in df_events["Type"]]
        df_events["event"] = [number_to_defintion_map[number].lower() for number in df_events["EventID"]]
